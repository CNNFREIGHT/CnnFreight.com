...
class Shop extends UserBase
{
    ...
    public function getOrderGoodsServiceList($param = [])
    {
        ...
        $Query = request()->param();
        // 查询订单数据
        $result = Db::name('shop_order_service')->field($field)->where($where)->order('add_time desc')->paginate(15, false, ['query' => $Query]);
    ...
    public function getOrderGoodsService($param = [])
    {
        ...
        $field = 'a.users_id, a.order_id, b.order_code, b.order_status, a.prom_type, a.details_id, a.product_id, a.product_name, a.product_price, a.num as product_num, a.litpic, a.apply_service, b.pay_time, b.consignee, b.mobile, b.province, b.city, b.district, b.address';
    ...
    public function addOrderGoodsService($param = [])
    {
        ...
        if (empty($formData['service_type'])) $this->error('请选择售后维权类型');
        ...
        $serviceData = Db::name('shop_order_service')->field('service_id, status')->where($where)->find();
        if (!empty($serviceData['service_id']) && !in_array($serviceData['status'], [3, 8])) $this->error('订单商品已申请售后');
        ...
        $addService = [
            ...
            'refund_code' => 'HH' . $this->times . rand(10,100),
            'add_time'    => $this->times,
            'update_time' => $this->times,
        ];
        // 售后逻辑变动统一处理
        $addService = model('ShopServiceHandle')->addServiceDataHandle($addService);

        $serviceID = 0;
        // 更新售后详情单
        if (!empty($serviceData['service_id']) && in_array($serviceData['status'], [3])) {
            $addService['status'] = 1;
            $resultID = Db::name('shop_order_service')->where(['service_id' => $serviceData['service_id']])->update($addService);
            if (!empty($resultID)) $serviceID = intval($serviceData['service_id']);
        }
        // 新增售后详情单
        else {
            $serviceID = Db::name('shop_order_service')->insertGetId($addService);
        }
        ...
        if (!empty($serviceID)) {
            ...
            Db::name('shop_order_details')->where('details_id', $addService['details_id'])->update($update);

            // 添加订单服务记录
            OrderServiceLog($serviceID, $addService['order_id'], $addService['users_id'], 0, '会员提交' . $addService['service_type_text'] . '申请，待管理员审核！');
