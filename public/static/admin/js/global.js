...
function checkSupportUtf8mb4() {
    ...
    $.ajax({
        type : 'post',
        url : __root_dir__+"/login.php?m=admin&c=System&a=checkSupportUtf8mb4&lang="+__lang__,
        data : {_ajax: 1},
        dataType : 'json',
        success : function(res){
            layer.closeAll();
            if (1 === parseInt(res.code)) {
                layer.confirm('要开启特殊字符支持啦~不过为了数据安全，得先备份数据库才行哟！确认现在执行这个处理不？', {
                    ...
                    window.location.href = __root_dir__+"/login.php?m=admin&c=Tools&a=index&lang="+__lang__
                });
            } else {
                showUnifiedAlert(res.msg);
            }
